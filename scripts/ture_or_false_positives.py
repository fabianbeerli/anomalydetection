...
def load_anomalies_and_truth(algo, overlap_type):
    ...
    if not anomalies_file.exists() or not truth_file.exists():
        print(f"Missing files for {algo} {overlap_type}:")
...
def plot_for_overlap_type(overlap_type, save_path):
    ...
    for i, algo in enumerate(algos):
        anomalies, truth = load_anomalies_and_truth(algo, overlap_type)
        ...
        for idx, row in anomalies.iterrows():
            ...
            anomaly_key = f"anomaly_{int(row['subsequence_idx'])}"
            if anomaly_key not in truth:
                print(f"Warning: No entry for {anomaly_key} in {algo} {overlap_type} true/false positives file.")
                is_true = None
            else:
    ...
    ax.set_title(f"S&P500 Anomalies ({overlap_type})")
...
def main():
    ...
    print("Plots saved as sp500_anomalies_overlap.png and sp500_anomalies_nonoverlap.png")
